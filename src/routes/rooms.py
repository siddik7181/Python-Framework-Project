...
async def create_room(room: RoomBase, session: AsyncSession = Depends(get_db), current_user: UserResponse = Depends(get_requesting_user)):
...
async def send_message(room_id: str, message: MessageBase, session: AsyncSession = Depends(get_db), current_user: UserResponse = Depends(get_requesting_user)):
...
async def delete_message(room_id: str, message_id: str, current_user: UserResponse = Depends(get_requesting_user), session: AsyncSession = Depends(get_db)):
